...
class SAM(nn.Module):
    def __init__(self, n_feat, chans, kernel_size, bias):
        super(SAM, self).__init__()
        self.conv1 = conv(n_feat, n_feat, kernel_size, bias=bias)
        self.conv2 = conv(n_feat, chans, kernel_size, bias=bias)
        self.conv3 = conv(chans, n_feat, kernel_size, bias=bias)
...
class MPRNet(nn.Module):
    def __init__(self, in_c=3, out_c=3, n_feat=96, scale_unetfeats=48, scale_orsnetfeats=32, num_cab=8, kernel_size=3, reduction=4, bias=False):
        ...
        self.sam12 = SAM(n_feat, in_c, kernel_size=1, bias=bias)
        self.sam23 = SAM(n_feat, in_c, kernel_size=1, bias=bias)
    ...
    def forward(self, x3_img):
        ...
        x2top_img  = x3_img[:,:,0:int(H_/2),:]
        x2bot_img  = x3_img[:,:,int(H_/2):H_,:]

        # Four Patches for Stage 1
        x1ltop_img = x2top_img[:,:,:,0:int(W_/2)]
        x1rtop_img = x2top_img[:,:,:,int(W_/2):W_]
        x1lbot_img = x2bot_img[:,:,:,0:int(W_/2)]
        x1rbot_img = x2bot_img[:,:,:,int(W_/2):W_]
