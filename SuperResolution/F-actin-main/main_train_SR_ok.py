    ...
    # for epoch in range(100000):
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 or current_step == 1:
                    ...
                    if len(E_imgs.shape) == 3:
                        for image_path, L_img, E_img, H_img in zip(test_data['H_path'], L_imgs, E_imgs, H_imgs):
                            ...
                            if not os.path.exists(image_mode_metrics):
                                write_csv.write_lines(image_mode_metrics, f"{'EPOCH'},{'MSE'},{'MAE'},{'PSNR'},{'SSIM'}\n")
                            ...
                            write_csv.write_lines(image_mode_metrics, f"{epoch:04d},{current_mse},{current_mae},{current_psnr},{current_ssim}\n")
                ...
                logger.info('<epoch:{:3d}, iter:{:8,d}, Average LOSS : {:<.4f}, Average PSNR : {:<.2f}dB,  Average SSIM : {:<.4f}, Average MSE : {:<.4f}, Average MAE : {:<.4f}\n'
                .format(epoch, current_step, avg_loss, avg_psnr, avg_ssim, avg_mse, avg_mae))
                write_csv.write_lines(avg_metrics, f"{epoch},{avg_mse},{avg_mse},{avg_psnr},{avg_ssim},{avg_loss}\n")
