...
def parse(opt_path, is_train=True):
    ...
    for phase, dataset in opt['datasets'].items():
        ...
        if 'dataroot_SH' in dataset and dataset['dataroot_SH'] is not None:
            dataset['dataroot_SH'] = os.path.expanduser(dataset['dataroot_SH'])
        if 'dataroot_SL' in dataset and dataset['dataroot_SL'] is not None:
            dataset['dataroot_SL'] = os.path.expanduser(dataset['dataroot_SL'])
...
def find_last_checkpoint(save_dir, net_type='G', pretrained_path=None):
    ...
    if file_list:
        ...
        for file_ in file_list:
            iter_current = re.findall(r"{}_(\d+).pth".format(net_type), file_)
            iter_exist.append(int(iter_current[0]))
        init_iter = max(iter_exist)
        init_path = os.path.join(save_dir, '{}_{}.pth'.format(net_type, init_iter))
    else:
        init_iter = 0
...
def save(opt, filename):
    opt_path = opt['opt_path']
    opt_path_copy = opt['path']['options']
    dirname, filename_ext = os.path.split(opt_path)
    textname, ext = os.path.splitext(filename_ext)
    dump_path = os.path.join(opt_path_copy, filename+get_timestamp()+ext)
    with open(dump_path, 'w') as dump_file:
        json.dump(opt, dump_file, indent=2)
