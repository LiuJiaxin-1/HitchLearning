...
def main(mode, image_name, srcname, root, finetune=False, image_type='F_actin', dataseed=0, test_dataroot=None, train_img_nums=0, test_img_nums=0,json_path='./options/train_F-actin_x2_psnr.json'):
    ...
    opt['path']['logs'] = os.path.join(root, 'logs')
    opt['path']['options'] = os.path.join(root, 'options')
    opt['path']['models'] = os.path.join(root, 'models', image_name)
    opt['path']['images'] = os.path.join(root, 'images')
    opt['path']['metrics'] = os.path.join(root, 'metrics')
    if opt['rank'] == 0:
        util.mkdirs((path for key, path in opt['path'].items() if 'pretrained' not in key))
    best_images_dir = os.path.join(opt['path']['images'], image_type + '_BEST')
    util.mkdir(best_images_dir)
    ...
    if finetune:
        if mode in ['Finetune', 'ours', 'SrcTrg'] and not opt['path']['pretrained_netG'] and not opt['path']['pretrained_netE']:
            ...
            # opt['path']['pretrained_optimizerG'] = init_path_optimizerG
            opt['path']['pretrained_netG'] = os.path.join(model_path, 'G_best.pth')
            opt['path']['pretrained_netE'] = os.path.join(model_path, 'E_best.pth')
            opt['path']['pretrained_optimizerG'] = os.path.join(model_path, 'optimizerG_best.pth')
            current_step = 0
    ...
    if opt['rank'] == 0:
        logger_name = 'train'
        utils_logger.logger_info(logger_name, os.path.join(opt['path']['logs'], logger_name + '_' + image_name + '_' + mode + '.log'))
    ...
    if opt['rank'] == 0:
        images_metrics = os.path.join(metrics_path, image_type + '_' + mode + '.csv')
    ...
    #         write_csv.write_lines(mode_metrics, f"{'ImageName'},{'MSE'},{'MAE'},{'PSNR'},{'SSIM'}\n")
    #     if not os.path.exists(images_metrics):
    #         write_csv.write_lines(images_metrics, f"{'ImageName'},{'Mode'},{'MSE'},{'MAE'},{'PSNR'},{'SSIM'}\n")
    #     if not os.path.exists(avg_metrics):
    #         write_csv.write_lines(avg_metrics, f"{'EPOCH'},{'AVGMSE'},{'AVGMAE'},{'AVGPSNR'},{'AVGSSIM'},{'LOSS'}\n")
    ...
    # for epoch in range(100000):
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 or current_step == 1:
                # -------------------------------
                # 7) testing
                # -------------------------------
                avg_psnr = 0.0
                ...
                for test_data in test_loader:
                    ...
                    if len(E_imgs.shape) == 3:
                        for image_path, L_img, E_img, H_img in zip(test_data['H_path'], L_imgs, E_imgs, H_imgs):
                            ...
                            image_mode_metrics = os.path.join(metrics_path, img_name + '_' + mode + '.csv')
                            if not os.path.exists(image_mode_metrics):
                                write_csv.write_lines(image_mode_metrics, f"{'STEP'},{'MSE'},{'MAE'},{'PSNR'},{'SSIM'}\n")
                            ...
                            write_csv.write_lines(image_mode_metrics, f"{current_step:04d},{current_mse},{current_mae},{current_psnr},{current_ssim}\n")
                            logger.info('{:->4d}--> {:>10s} | PSNR:{:<4.2f}dB | SSIM:{:<4.4f} | MSE:{:<4.5f} | MAE:{:<4.5f}'
                            .format(idx, image_name_ext, current_psnr, current_ssim, current_mse, current_mae,))
                ...
                logger.info("############################################################")
                logger.info('[curr_epoch:{:3d}, step:{:8,d}, Avrg_PSNR : {:<.2f}dB, Avrg_SSIM : {:<.4f}, Avrg_MSE : {:<.4f}, Avrg_MAE : {:<.4f}]\n'
                .format(epoch, current_step, avg_psnr, avg_ssim, avg_mse, avg_mae))
                logger.info("[best_epoch:{:3d}, step:{:8,d}, Best_PSNR : {:<.2f}dB, Best_SSIM : {:<.4f}, Best_MSE : {:<.4f}, Best_MAE : {:<.4f}]\n"
                .format(best_epoch, best_step, best_psnr, best_ssim, best_mse, best_mae))
                logger.info("############################################################")
    logger.info('<Best epoch:{:3d}, Best step:{:8,d}, Best PSNR : {:<.2f}dB,  Best SSIM : {:<.4f}, Best MSE : {:<.4f}, Best MAE : {:<.4f}\n'
    .format(best_epoch, best_step, best_psnr, best_ssim, best_mse, best_mae))
    ...
    for j in range(len(test_set.paths_L)):
        ...
        img_name, ext = os.path.splitext(image_name_ext)
        img_name = img_name.split('_', 1)[-1]
        write_csv.write_lines(images_metrics, f"{img_name},{mode},{best_mse_all[j]},{best_mae_all[j]},{best_psnr_all[j]},{best_ssim_all[j]}\n")
        # img_path = os.path.join(opt['path']['images'], img_name, '{:s}_{:d}_{:s}_E.tif'.format(img_name, best_step, mode))
        # best_path = os.path.join(best_images_dir, 'Best_{:s}_{:s}.tif'.format(img_name, mode))
