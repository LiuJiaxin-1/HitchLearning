...
def main(mode, image_name, srcname, image_type, dataseed=0, test_dataroot=None, train_img_nums=0, test_img_nums=0,json_path='./options/train_F-actin_x2_psnr.json'):
    ...
    if not mode == 'Src':
        opt['datasets']['update']['dataroot_L'] = opt['datasets']['train']['dataroot_L']
        opt['datasets']['update']['dataroot_H'] = opt['datasets']['train']['dataroot_H'].replace('SudoSrc'+srcname, 'SR')
    ...
    if mode == 'Finetune':
        ...
        model_path = model_path.replace('Finetune', 'Src')
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            pass
            # update_set = define_Dataset(dataset_opt, train_nums[mode], dataseed)
            # if opt['dist']:
            #     train_sampler = DistributedSampler(train_set, shuffle=dataset_opt['dataloader_shuffle'], drop_last=True, seed=seed)
            #     update_loader = DataLoader(update_set,
            #                                batch_size=dataset_opt['dataloader_batch_size']//opt['num_gpu'],
            #                                shuffle=False,
            #                                num_workers=dataset_opt['dataloader_num_workers']//opt['num_gpu'],
            #                                drop_last=False,
    ...
    # for epoch in range(100000):
        ...
        '''
        # -------------------------------
        # 6) update target SR images
        # -------------------------------
        if epoch % opt['train']['checkpoint_save'] == 0 and avg_loss > total_loss/(i+1):
            # if mode in ['AllTrg', 'PartTrg', 'Trg', 'Finetune', 'ours', 'SrcTrg']:
            if mode not in ['Src']:
                for update_data in update_loader:
                    model.feed_data(data=update_data, train_flag=False)
                    model.test()
                    visuals = model.current_results()
                    E_img = util.tensor2numpy(visuals['E'])
                    H_img = util.tensor2numpy(visuals['H'])
                    if len(E_img.shape) == 3:
                        for index, sudo_sr, sr in zip(update_data['index'], E_img, H_img):
                            sudo_sr_path = update_set.paths_H[index].replace('SudoSrc' + srcname, mode)
                            sr_path = update_set.paths_H[index].replace('SudoSrc' + srcname, mode+'_sr')
                            util.mkdir(os.path.dirname(sudo_sr_path))
                            util.mkdir(os.path.dirname(sr_path))
                            # sudo_sr = util.preprocess(E_img, False, False)
                            sudo_sr_norm = util.preprocess(sudo_sr, False, False)
                            sudo_sr_norm_psnr = calculate_psnr(sr, sudo_sr_norm)
                            sudo_sr_psnr = calculate_psnr(sr, sudo_sr)
                            util.tifsave(sudo_sr, sudo_sr_path)
                            util.tifsave(sr, sr_path)
                            train_set.paths_H[index] = train_set.paths_H[index].replace('SudoSrc'+srcname, mode)
                            update_set.paths_H[index] = update_set.paths_H[index].replace('SudoSrc'+srcname, mode)
                    elif len(E_img.shape) == 2:
                        index = update_data['index'][0]
                        # sudo_sr = util.preprocess(E_img, False, False)
                        sudo_sr = E_img
                        sr = H_img
                        sudo_sr_norm = util.preprocess(sudo_sr, False, False)
                        sudo_sr_norm_psnr = calculate_psnr(sr, sudo_sr_norm)
                        sudo_sr_psnr = calculate_psnr(sr, sudo_sr)
                        sudo_sr_path = update_set.paths_H[index].replace('SR', mode)
                        sr_path = update_set.paths_H[index].replace('SR', mode +'_sr')
                        util.mkdir(os.path.dirname(sudo_sr_path))
                        util.mkdir(os.path.dirname(sr_path))
                        util.tifsave(sudo_sr, sudo_sr_path)
                        util.tifsave(sr, sr_path)
                        train_set.paths_H[index] = train_set.paths_H[index].replace('SudoSrc'+srcname, mode)
                        # update_set.paths_H[index] = update_set.paths_H[index].replace('SR', mode)
                if mode in ['ours', 'SrcTrg']:
                    for j in range(len(train_set.paths_H)):
                        train_set.paths_H[j] = train_set.paths_H[j].replace('SudoSrc'+srcname, mode)
        '''                
...
if __name__ == '__main__':
    # test_data = ''
    # main(mode='Finetune', image_name='F_actin', image_type='F_actin', test_dataroot='', test_img_nums=0, train_img_nums=10, dataseed=1)
    # import utils.utils_image as util
    # modes = ['AllTrg', 'PartTrg', 'Trg', 'Src', 'Finetune', 'SrcTrg', 'ours']
    # test_root_L = "/data/ljx/data/SUFDD/F_actin/9/Val/Noise" 
    # test_root_H = "/data/ljx/data/SUFDD/F_actin/9/Val/SR" 
    ...
    #     for mode in modes:
    #         if i and mode in ['AllTrg', 'PartTrg']:
    #             continue
    ...
    #         main(mode=mode, image_name=image_name, test_dataroot=test_data, test_img_nums=0, train_img_nums=10, dataseed=1)
    import os
    import utils.utils_image as util
