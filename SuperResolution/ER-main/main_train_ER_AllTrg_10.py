'''
使用ER Val集中的SudoER-D4作为sudoSR
每个10个step测试一次
json文件在run-d4中
'''
import os
import utils.utils_image as util
from main_train_SR_10 import main
json_root = './options/ER/run-d4/train_ER_x2_'
image_type = 'ER'
# modes = ['AllTrg', 'PartTrg', 'Trg', 'Src', 'Finetune', 'SrcTrg', 'ours']
modes = ['AllTrg']
test_root_L = "/data/liujiaxin/data/SUFDD/ER/9/Val/Noise" 
test_root_H = "/data/liujiaxin/data/SUFDD/ER/9/Val/SR/" 
test_paths_L = util.get_image_paths(test_root_L)
test_paths_H = util.get_image_paths(test_root_H)
roots = ['04', '05', '06', '07', '08', '09']
dir = "/data/liujiaxin/Results/SR/ER/"
for root in roots:
    root = os.path.join(dir, root, 'AllTrg')
    for i in range(len(test_paths_L)):
        ...
        for mode in modes:
            ...
            main(mode=mode, image_name=image_name, srcname='ER-D4', finetune=False, root=root, image_type=image_type, 
                 test_dataroot=test_data, test_img_nums=0, train_img_nums=10, dataseed=1, json_path=json_path)